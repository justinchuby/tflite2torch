...
class OperatorConverter:
    ...
    def convert(self, op_type: str, inputs: list[Any], options: dict[str, Any]) -> Callable:
        """
    ...
    def _simple_call_function(self, target: Callable, with_activation: bool = False) -> Callable:
        ...
        def converter(inputs: list[Any], options: dict[str, Any]) -> Callable:
            activation = (
                options.get("fused_activation_function", "NONE") if with_activation else "NONE"
            )
        ...
        def converter(inputs: list[Any], options: dict[str, Any]) -> Callable:
            ...
            def build_graph(
                ...
                output_node = graph.call_module(
                    module_name, args=(input_nodes[0],) if input_nodes else ()
                )
                output_node.name = node_name
                return output_node
            ...
            if len(operator.inputs) >= 2:
                ...
                if len(weight_tensor_info.shape) == 4:
                    ...
                    output_node = graph.call_function(
                        torch.permute, args=(conv_output, (0, 2, 3, 1))
                    )
                    output_node.name = node_name
    ...
    def _convert_depthwise_conv2d(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
    ...
    def _convert_fully_connected(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
    ...
    def _convert_max_pool2d(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
        ...
        def build_graph(
            ...
            # Convert back from NCHW to NHWC
            output_node = graph.call_function(torch.permute, args=(pool_output, (0, 2, 3, 1)))
            output_node.name = node_name
            return output_node
    ...
    def _convert_reshape(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
        ...
        def build_graph(
            ...
            if len(input_nodes) >= 2:
                ...
                if not input_nodes:
                    raise ValueError(
                        f"RESHAPE operator at {node_name} requires at least one input tensor"
                    )
    ...
    def _convert_concatenation(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
        ...
        def build_graph(
            ...
            if len(input_nodes) > 0:
                output_node = graph.call_function(
                    torch.cat, args=(tuple(input_nodes),), kwargs={"dim": axis}
                )
            else:
                output_node = graph.call_function(lambda *args: args[0] if args else None, args=())
            output_node.name = node_name
            return output_node
        ...
        def build_graph(
            ...
            if len(input_nodes) >= 2:
                ...
                if perm_idx in weights:
                    ...
                    output_node = graph.call_function(lambda x: x, args=(input_node,))
            else:
                output_node = graph.call_function(
                    lambda x: x, args=(input_nodes[0],) if input_nodes else ()
                )
            output_node.name = node_name
            return output_node
        ...
        def build_graph(
            ...
            def convert_pad(input_tensor, pad_tensor):
                ...
                return torch.nn.functional.pad(
                    input_tensor, tuple(pad_list), mode="constant", value=0
                )
    ...
    def _convert_squeeze(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
        ...
        def build_graph(
            ...
            output_node = graph.call_function(
                torch.squeeze, args=tuple(input_nodes), kwargs={"dim": squeeze_dims}
            )
            output_node.name = node_name
            return output_node
    ...
    def _convert_gather(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
        ...
        def build_graph(
            ...
            output_node = graph.call_function(
                torch.index_select, args=(input_nodes[0], axis, input_nodes[1])
            )
            output_node.name = node_name
            return output_node
    ...
    def _convert_batch_to_space(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
    ...
    def _convert_space_to_batch(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
    ...
    def _convert_resize_bilinear(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
    ...
    def _convert_resize_nearest(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
        ...
        def build_graph(
            ...
            if len(operator.inputs) >= 2:
                ...
                if len(weight_tensor_info.shape) == 5:
                    kernel_size = (
                        weight_tensor_info.shape[0],
                        weight_tensor_info.shape[1],
                        weight_tensor_info.shape[2],
                    )
                    in_channels = weight_tensor_info.shape[3]
                    ...
                    permute_to_ncdhw = graph.call_function(
                        torch.permute, args=(input_nodes[0], (0, 4, 1, 2, 3))
                    )
                    ...
                    output_node = graph.call_function(
                        torch.permute, args=(output_node, (0, 2, 3, 4, 1))
                    )
                ...
                output_node = (
                    input_nodes[0] if input_nodes else graph.call_function(lambda: None, args=())
                )
                output_node.name = node_name
            return output_node
        ...
        def build_graph(
            ...
            if len(operator.inputs) >= 2:
                ...
                if len(weight_tensor_info.shape) == 4:
                    ...
                    permute_to_nchw = graph.call_function(
                        torch.permute, args=(input_tensor, (0, 3, 1, 2))
                    )
                    ...
                    # Convert back from NCHW to NHWC
                    output_node = graph.call_function(
                        torch.permute, args=(output_node, (0, 2, 3, 1))
                    )
    ...
    def _convert_l2_normalization(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
        ...
        def build_graph(
            ...
            output_node = graph.call_function(
                torch.nn.functional.normalize, args=tuple(input_nodes), kwargs={"p": 2, "dim": -1}
            )
            output_node.name = node_name
            return output_node
    ...
    def _convert_reduce_prod(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
        ...
        def build_graph(
            ...
            # Fallback
            output_node = graph.call_function(
                torch.prod, args=(input_nodes[0],), kwargs={"keepdim": keep_dims}
            )
            output_node.name = node_name
            return output_node
    ...
    def _convert_reduce_any(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
        ...
        def build_graph(
            ...
            # Fallback
            output_node = graph.call_function(
                torch.any, args=(input_nodes[0],), kwargs={"keepdim": keep_dims}
            )
            output_node.name = node_name
            return output_node
    ...
    def _convert_broadcast_args(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
        ...
        def build_graph(
            ...
            output_node = graph.call_function(
                torch.broadcast_shapes, args=(shape1_tuple, shape2_tuple)
            )
            output_node.name = node_name
            return output_node
    ...
    def _convert_depth_to_space(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
        ...
        def build_graph(
            ...
            # Convert back from NCHW to NHWC
            output_node = graph.call_function(torch.permute, args=(shuffle_output, (0, 2, 3, 1)))
            output_node.name = node_name
            return output_node
    ...
    def _convert_mirror_pad(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
        ...
        def build_graph(
            ...
            output_node = graph.call_function(
                convert_pad_tflite_to_torch, args=(input_tensor, pad_tensor)
            )
            output_node.name = node_name
            return output_node
    ...
    def _convert_padv2(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
        ...
        def build_graph(
            ...
            def convert_padv2(input_tensor, pad_tensor, constant_value=0):
                ...
                return torch.nn.functional.pad(
                    input_tensor, tuple(pad_list), mode="constant", value=constant_value
                )

            if constant_value is not None:
                output_node = graph.call_function(
                    convert_padv2, args=(input_tensor, pad_tensor, constant_value)
                )
            else:
    ...
    def _convert_space_to_depth(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
        ...
        def build_graph(
            ...
            # Convert back from NCHW to NHWC
            output_node = graph.call_function(torch.permute, args=(unshuffle_output, (0, 2, 3, 1)))
            output_node.name = node_name
            return output_node
    ...
    def _convert_strided_slice(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
        ...
        def build_graph(
            ...
            if strides is not None:
                output_node = graph.call_function(
                    strided_slice_impl, args=(input_tensor, begin, end, strides)
                )
            else:
                output_node = graph.call_function(
                    strided_slice_impl, args=(input_tensor, begin, end)
                )
            output_node.name = node_name
            return output_node
    ...
    def _convert_topk_v2(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
        ...
        def build_graph(
            ...
            topk_output = graph.call_function(
                torch.topk, args=(input_nodes[0],), kwargs={"k": k_value}
            )
    ...
    def _convert_unpack(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
        ...
        def build_graph(
            ...
            output_node = graph.call_function(
                torch.unbind, args=tuple(input_nodes), kwargs={"dim": axis}
            )
            output_node.name = node_name
            return output_node
    ...
    def _convert_one_hot(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
        ...
        def build_graph(
            ...
            if depth is not None:
                ...
                output_node = graph.call_function(
                    torch.nn.functional.one_hot, args=(indices_long, depth)
                )
            else:
    ...
    def _convert_bidirectional_sequence_lstm(
        self, inputs: list[Any], options: dict[str, Any]
    ) -> Callable:
    ...
    def _convert_unidirectional_sequence_lstm(
        self, inputs: list[Any], options: dict[str, Any]
    ) -> Callable:
    ...
    def _convert_bidirectional_sequence_rnn(
        self, inputs: list[Any], options: dict[str, Any]
    ) -> Callable:
    ...
    def _convert_unidirectional_sequence_rnn(
        self, inputs: list[Any], options: dict[str, Any]
    ) -> Callable:
    ...
    def _convert_fake_quant(self, inputs: list[Any], options: dict[str, Any]) -> Callable:
        ...
        def build_graph(
            ...
            output_node = graph.call_function(
                torch.fake_quantize_per_tensor_affine, args=tuple(input_nodes)
            )
            output_node.name = node_name
            return output_node
    ...
    def get_activation_module(self, activation: str) -> nn.Module | None:
        """
